...
def downscale_images(hr_dir, scale):
    lr_images = {}
    for filename in os.listdir(hr_dir):
        if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp')):
            ...
            lr_image = cv2.resize(hr_image, new_dimensions, interpolation=cv2.INTER_AREA)
            lr_images[filename] = lr_image
    return lr_images
...
def main():
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            if dataset_opt['dataroot_LQ'] is None:
                dataset_opt['lr_images'] = downscale_images(dataset_opt['dataroot_GT'], opt['scale'])
            dataset_opt['pin_memory'] = True  # Enable pinned memory
            dataset_opt['num_workers'] = 0  # Adjust the number of workers
